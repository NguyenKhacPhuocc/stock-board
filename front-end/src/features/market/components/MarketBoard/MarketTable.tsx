...
import { selectFilteredStocks } from "../../marketSelectors";
...
const StockRow = memo(({ item }: { item: StockInstrument }) => {
  ...
  return (
    <tr>
      <td>
        <PinIcon className={styles.iconPin} />
      ...
      <td className={getColorClass(bidPrice3, ref, ceil, flr)}>{formatVol(bidVol3)}</td>
      <td className={getColorClass(bidPrice2, ref, ceil, flr)}>{formatPrice(bidPrice2)}</td>
      <td className={getColorClass(bidPrice2, ref, ceil, flr)}>{formatVol(bidVol2)}</td>
      <td className={getColorClass(bidPrice1, ref, ceil, flr)}>{formatPrice(bidPrice1)}</td>
      <td className={getColorClass(bidPrice1, ref, ceil, flr)}>{formatVol(bidVol1)}</td>

      {/* Matching */}
      <td className={getColorClass(lastPrice, ref, ceil, flr)}>{formatPrice(lastPrice)}</td>
      <td className={getColorClass(lastPrice, ref, ceil, flr)}>{formatVol(closeVol)}</td>
      ...
      <td className={getColorClass(offerPrice1, ref, ceil, flr)}>{formatVol(offerVol1)}</td>
      <td className={getColorClass(offerPrice2, ref, ceil, flr)}>{formatPrice(offerPrice2)}</td>
      <td className={getColorClass(offerPrice2, ref, ceil, flr)}>{formatVol(offerVol2)}</td>
      <td className={getColorClass(offerPrice3, ref, ceil, flr)}>{formatPrice(offerPrice3)}</td>
      <td className={getColorClass(offerPrice3, ref, ceil, flr)}>{formatVol(offerVol3)}</td>
...
export default function MarketTable() {
  ...
  const stocks = useAppSelector(selectFilteredStocks);
  ...
  return (
    <div className={styles.tableWrapper}>
      <table>
        <thead>
          <tr>
            <th rowSpan={2} style={{ width: "100px" }}>Mã CK</th>
            <th rowSpan={2} style={{ width: "40px" }} className={styles.colorRef}>TC</th>
            <th rowSpan={2} style={{ width: "40px" }} className={styles.colorCeiling}>Trần</th>
            <th rowSpan={2} style={{ width: "40px" }} className={styles.colorFloor}>Sàn</th>
            <th colSpan={6}>Mua</th>
        ...
        <tbody>
          {stocks.map((item: StockInstrument) => (
